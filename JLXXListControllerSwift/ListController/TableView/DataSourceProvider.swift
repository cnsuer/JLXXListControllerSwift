...
public class DataSourceProvider: NSObject {
	...
	*/
	...
	所有section的数据
	*/
	public var sectionItems: Array<TableViewSectionItem> = []
	
	public var headerItem: Any?

	public var footerItem: Any?
	
	init(dataSource: DataSourceProtocol, delegate: AnyObject?) {
	...
	*/
	public func addSectionItem(count: Int) {
		for _ in 0..<count {
			sectionItems.append(TableViewSectionItem())
		}
	}
	...
	*/
	public func getSectionItem(at section: Int) -> TableViewSectionItem {
...
extension DataSourceProvider: UITableViewDataSource {
	...
	public func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
		...
		if editingStyle == .delete {
			let item = rowItem(for: tableView, at: indexPath)
			let isChange = dataSource.deleteRow(for: tableView, with: item, at: indexPath)
			
			if isChange, let items = rowItems(for: tableView, at: indexPath) {
